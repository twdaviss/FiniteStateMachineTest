...
namespace StateManager {
    [CreateAssetMenu(menuName = "States/Character/Starved")]
    public class StarvedState : State<CharacterController>
    {
        ...
        {
            Debug.Log("Time of Inti " + Time.time);
            base.Init(parent);
            parent.GetComponentInChildren<SpriteRenderer>().color = Color.magenta;
        ...
        public override void Update()
        {
            ...
            if (target == null)
            {
                ...
                for (int i = 0; i < foods.transform.childCount; i++)
                {
                    ...
                    if ((foods.transform.GetChild(i).position - transform.position).magnitude < (target.position - transform.position).magnitude)
                    {
                        target = foods.transform.GetChild(i);
                    }
                }
